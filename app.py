...
try:
    ...
    GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "AIzaSyCH-3Qfhnm8WdOxBNZq8nJHWQk-wyxiXc8")
...
try:
    ...
    GROQ_API_KEY = os.getenv("GROQ_API_KEY", "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")
    ...
    groq_client = None
KAGGLE_API_URL = os.getenv("KAGGLE_API_URL", "https://coveted-crafter-unmolded.ngrok-free.dev")
# ─── App Initialization ───────────────────────────────────────────────
...
def create_car(car: CarAnnonce, authorization: str = Header(None)):
    ...
    try:
        with engine.connect() as conn:
            ...
            """), {**car.model_dump(), "user_id": user_id, "vendeur_nom": vendeur_nom, "vendeur_telephone": vendeur_telephone, "images": json.dumps(car.images) if car.images else None})
            conn.commit()
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(text("""
                ...
                VALUES (:nom, :prenom, :email, :password_hash, :tel) RETURNING id, nom, prenom, email, avatar_url
            """), {"nom": nom, "prenom": prenom, "email": email, "password_hash": hash_password(password), "tel": telephone})
            conn.commit()
            row = result.fetchone()
...
def send_message(msg: MessageCreate, authorization: str = Header(None)):
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.execute(text("""
    INSERT INTO messages (sender_id, receiver_id, car_id, content, is_read) 
    VALUES (:s, :r, :c, :content, FALSE)
