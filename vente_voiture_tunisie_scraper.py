...
class VenteVoitureTunisieScraper:
    ...
    def scrape_all_pages(self, max_cars=1000, max_pages=100):
        ...
        while len(self.data) < max_cars and page_num <= max_pages:
            logger.info(f"\nPage {page_num}")
            ...
            for idx, car_url in enumerate(links, 1):
                if len(self.data) >= max_cars:
                    logger.info(f"   Reached max cars limit ({max_cars})")
                    return
            ...
            if not self.load_page_with_retry(self.LISTING_URL):
                logger.error("Failed to reload listing page")
                break
    ...
    def run(self, max_cars=1000, max_pages=100):
        logger.info("\n" + "="*70)
        logger.info("🚗 VENTE-VOITURE-TUNISIE.COM SCRAPER")
        logger.info(f"   Max cars: {max_cars}, Max pages: {max_pages}")
        logger.info("="*70 + "\n")
        
        self.setup_driver()
        
        try:
            self.scrape_all_pages(max_cars=max_cars, max_pages=max_pages)
            self.save_data()
